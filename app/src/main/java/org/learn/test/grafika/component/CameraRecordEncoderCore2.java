    ...
    public void drainEncoder(boolean endOfStream) {
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (!endOfStream) {
                    if (VERBOSE) Log.d(TAG, "Video no output available, out of while(true) ");
                    break;      // out of while(true){}
                } else {
                    if (VERBOSE) Log.d(TAG, "Video no output available, spinning to await EOS");
                }
                ...
                if (VERBOSE) Log.d(TAG, "mVideoEncoder INFO_OUTPUT_FORMAT_CHANGED, mVideoTrackIndex:"+mVideoTrackIndex);
                ...
                if ((mVideoBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    ...
                    if (VERBOSE) Log.d(TAG, "ignoring Video BUFFER_FLAG_CODEC_CONFIG");
                ...
                if (mVideoBufferInfo.size != 0) {
                    if (mMuxerStarted) {
                        ...
                        if((mVideoBufferInfo.flags & MediaCodec.BUFFER_FLAG_KEY_FRAME) != 0){
                            if (VERBOSE) Log.i(TAG, "sent BUFFER_FLAG_KEY_FRAME Video bytes to mMuxer");
                        }
                ...
                if ((mVideoBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    if (!endOfStream) {
                        ...
                        if (VERBOSE) Log.d(TAG, "Video end of stream reached");
                    }
                    break;      // out of while
                }
            }
        ...
        while ( !inputDone ) {
            ...
            //if (!inputDone) {
                ...
                if (inputBufIndex >= 0) {
                    ...
                    if (VERBOSE) Log.d(TAG, "AudioEncoder input buffer not available");
                }
            //}
        }
    }
    ...
    public void drainAudioEncoder(boolean endOfStream) {
        ...
        while( true ){
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                // no output available yet
                if (!endOfStream) {
                    if (VERBOSE) Log.d(TAG, "Audio no output available, out of while(true) ");
                    break;      // out of while(true){}
                } else {
                    if (VERBOSE) Log.d(TAG, "Audio no output available, spinning to await EOS");
                }
            } else if (encoderStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                // not expected for an encoder
                ...
                if (VERBOSE) Log.d(TAG, "mAudioEncoder INFO_OUTPUT_FORMAT_CHANGED, mAudioTrackIndex:"+mAudioTrackIndex);
                ...
                if ((mAudioBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    if (VERBOSE) Log.d(TAG, "ignoring Audio BUFFER_FLAG_CODEC_CONFIG");
                    mAudioBufferInfo.size = 0;
                }
                if (mAudioBufferInfo.size != 0) {
                    if (mMuxerStarted) {
                        if((mAudioBufferInfo.flags & MediaCodec.BUFFER_FLAG_KEY_FRAME) != 0){
                            if (VERBOSE) Log.i(TAG, "sent BUFFER_FLAG_KEY_FRAME Audio bytes to mMuxer");
                        }
                ...
                if ((mAudioBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    if (!endOfStream) {
                        ...
                        if (VERBOSE) Log.d(TAG, "Audio end of stream reached");
                    }
                    break;      // out of while
                }
            }
