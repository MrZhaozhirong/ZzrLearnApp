package org.learn.test.grafika.content;
...
public abstract class GeneratedMovie implements Content {
    ...
    protected void prepareEncoder(String mimeType, int width, int height, int bitRate,
        ...
        // We're not actually interested in multiplexing audio.
        // We just want to convert the raw H.264 elementary stream we get from MediaCodec into a .mp4 file.
        if (DEBUG) Log.i(TAG, "output will go to " + outputFile);
    ...
    protected void releaseEncoder() {
        if (DEBUG) Log.i(TAG, "releasing encoder objects");
        if (mEncoder != null) {
            mEncoder.stop();
            mEncoder.release();
            mEncoder = null;
     ...
     * If it is set, we send EOS to the encoder, and then iterate until we see EOS on the output.
     * Calling this with endOfStream set should be done once, right before stopping the muxer.
     */
    protected void drainEncoder(boolean endOfStream) {
        final int TIMEOUT_USEC = 10000;
        if (DEBUG) Log.i(TAG, "drainEncoder(" + endOfStream + ")");
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (!endOfStream) {
                    ...
                    if (DEBUG) Log.i(TAG, "no output available, spinning to await EOS");
                }
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    ...
                    if (DEBUG) Log.i(TAG, "ignoring BUFFER_FLAG_CODEC_CONFIG");
                    mBufferInfo.size = 0;
                }
                if (mBufferInfo.size != 0) {
                    ...
                    if (DEBUG) Log.i(TAG, "sent " + mBufferInfo.size + " bytes to muxer");
                }
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    if (!endOfStream) {
                        ...
                        if (DEBUG) Log.i(TAG, "end of stream reached");
                    }
                    break;      // out of while
                }
            }
